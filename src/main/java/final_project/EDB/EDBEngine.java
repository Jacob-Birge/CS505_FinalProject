                        ...
                        try (Statement stmt = conn.createStatement()) {
                            try(ResultSet rs = stmt.executeQuery(queryString)) {
                                if(rs.next()) {
                                    ...
                                    String closestHosp = rs.getString("hid");
                                    map.put("closest_hospital", closestHosp);
                                    // Update Hospital Bed Count 
                                    Integer hospitalAsInt = Integer.parseInt(closestHosp);
                                    executeUpdate("UPDATE HOSPITALS SET used_beds = used_beds + 1 WHERE id=" + hospitalAsInt);
                                }
                                else {
                                    ...
                                    if (!hidForClosestHospital.equals("-1")){
                                        Integer hospitalAsInt = Integer.parseInt(hidForClosestHospital);
                                        executeUpdate("UPDATE HOSPITALS SET used_beds = used_beds + 1 WHERE id=" + hospitalAsInt);
                                    }
                                }
                            }
